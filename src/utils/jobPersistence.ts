...
type PersistencePrisma = {
  ...
  jobPosting: {
    upsert(args: Record<string, unknown>): Promise<{ id: string; company?: string | null }>;
...
async function syncFirm(args: {
  ...
  const firm = await args.prisma.firm.upsert({
    ...
    update: args.companyLogoUrl ? { logoUrl: args.companyLogoUrl } : {},
    create: {
      name: args.company,
  ...
  const [totalReviewedJobs, decisions] = await Promise.all([
    ...
    args.prisma.applicationDecision.findMany({
      where: {
        jobPosting: {
          firmId: firm.id,
        },
  ...
  }) as Promise<{ id: string; name: string; logoUrl?: string | null }>;
}
